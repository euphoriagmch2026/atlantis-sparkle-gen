...
const OrderConfirmation = () => {
  ...
  if (state) {
    return (
      <div className="min-h-screen bg-background relative overflow-hidden">
        <Helmet>
          <title>Order Confirmed | EUPHORIA 2026</title>
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden">
      <Helmet>
        <title>Find Order | EUPHORIA 2026</title>
        <meta name="robots" content="noindex, nofollow" />
      </Helmet>
      <FloatingParticles />
      <Navbar />
          ...
          {!lookupData ? (
            <div className="glass-card rounded-xl p-8 border border-border/50">
              <div className="text-center mb-6">
                ...
                <h1 className="font-cinzel text-2xl font-bold text-foreground text-glow">
                  Find Your Order
                </h1>
                <p className="text-muted-foreground text-sm mt-2">
                  Enter your Order ID and email to view your registration
                  details.
                </p>
              </div>
              <form onSubmit={handleLookup} className="space-y-4">
                ...
                >
                  {isLooking ? (
                    <>
                      <Loader2 className="w-4 h-4 mr-2 animate-spin" /> Looking
                      up...
                    </>
                  ) : (
                    "Find Order"
            ...
            <div className="text-center">
              ...
              <h1 className="font-cinzel text-2xl font-bold text-foreground mb-2 text-glow">
                Order Found
              </h1>
              <p className="text-muted-foreground mb-6">
                Status:{" "}
                <span className="text-primary font-semibold capitalize">
                  {lookupData.order.status}
                </span>
              </p>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between py-2 border-b border-border/30">
                    ...
                    <span className="font-mono text-primary text-sm">
                      {lookupData.order.razorpay_order_id}
                    </span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-border/30">
                    <span className="text-muted-foreground">Name</span>
                    <span className="text-foreground">
                      {lookupData.order.full_name}
                    </span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-border/30">
                    <span className="text-muted-foreground">College</span>
                    <span className="text-foreground">
                      {lookupData.order.college}
                    </span>
                  </div>
                  {lookupData.items.length > 0 && (
                    <>
                      <h3 className="font-cinzel text-sm text-foreground pt-2">
                        Items
                      </h3>
                  ...
                  <div className="flex justify-between py-2 border-t border-border/30 font-semibold">
                    ...
                    <span className="text-accent text-glow-gold">
                      ₹{(lookupData.order.total_amount / 100).toFixed(0)}
                    </span>
                  </div>
                </div>
              </div>
